...
3. 进入“算子中心”查看端口、参数、成熟度和可运行状态；算子中心不直接执行任务。
4. 点击“新建工作流”，默认选择 `Dataset Channel → Qscore`，也可以选择 S01 基础版或压力增强版。
...
8. S01 压力增强工作流完成后可以查看风险时间线、候选时段和报告；候选仍需人工核验。GPU Chronos-2 在 GPU 环境验收前保持禁用。

数据资产默认仅创建者与管理员可见。CSV 原件由后端保存到 MinIO，浏览器不会得到 MinIO 凭据或直连下载地址；只读 MySQL URI 输入框也不会回显密码。

## 工作流与算子页面

登录后进入“算子中心”查看受审核的算子契约和内置结构。进入“工作流”可以从草稿、已发布版本或内置模板创建流程；Rete 编辑器支持节点目录点击/拖入、节点拖动、端口连线、输出端口声明、参数类型控件、画布适应、撤销/重做和 Graph 序列化。保存、校验和发布仍由后端裁决。

新建流程默认使用 `minimal_quality_check@1.0.0`。S01 模板会展开为普通可编辑节点，绑定数据资产后再运行。工作流运行记录详情页展示节点状态、参数快照、日志、artifact 和最终结果，刷新后可从 REST 接口恢复状态。
...
src/app/features/   登录、首页、数据资产、算子中心、工作流、任务、结果、用户与运行详情
